    ...
    match msg {
        ...
        WM_COMMAND => { 
            ...
            if control_id == LINK_BUTTON_ID as usize { 
                let url = windows::core::w!("https://github.com/ReflexDevelopment/IndiRPC");
...
fn main() {
    ...
    if config.hidden {
        // No window, no tray — just exist as a process until taskkill terminates us
        std::thread::park();
        return;
    }
    ...
    let _app_label_hwnd = create_label(hwnd, "IndiRPC", instance, 10, 10, None, 20);
    
        let _app_label_hwnd = create_label(hwnd, "This program is part of IndiRPC", instance, 10, 60, None, 20);
